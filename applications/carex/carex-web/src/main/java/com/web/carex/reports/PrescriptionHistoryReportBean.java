...
@Named("prescriptionHistoryReportBean")
@Scope("session")
public class PrescriptionHistoryReportBean extends CarexManagedBean implements Serializable {
    ...
    private List<PrescriptionHistoryRow> prescriptionRows = new ArrayList<>();
    ...
    public void onOrganizationChange() {
        ...
        prescriptionRows = new ArrayList<>();
    }
    ...
    public void fetchReport() {
        if (selectedOrganizationId == null) {
            addMessage(FacesMessage.SEVERITY_ERROR, "Organization required", "Select an organization to fetch prescription history.");
            prescriptionRows = new ArrayList<>();
            return;
        }
        if ("SPECIFIC".equals(patientScope) && selectedPatientId == null) {
            addMessage(FacesMessage.SEVERITY_ERROR, "Patient required", "Choose a patient for specific prescription history.");
            prescriptionRows = new ArrayList<>();
            return;
        }
        ...
        List<PrescriptionHistoryRow> rows = new ArrayList<>();
        for (Consultation consultation : consultations) {
            ...
            for (ConsultationMedicine medicine : consultation.getConsultationMedicines()) {
                PrescriptionHistoryRow row = new PrescriptionHistoryRow();
                row.setConsultation(consultation);
                row.setConsultationMedicine(medicine);
                rows.add(row);
            }
        }
        rows.sort(Comparator.comparing((PrescriptionHistoryRow row) -> row.getConsultation().getConsultationDate()).reversed()
        ...
        if (prescriptionRows.isEmpty()) {
            addMessage(FacesMessage.SEVERITY_INFO, "No data", "No prescription lines matched the selected criteria.");
        }
    }
    ...
    public int getPrescriptionLineCount() {
        return prescriptionRows.size();
    }

    public int getConsultationCount() {
        return (int) prescriptionRows.stream()
                .map(row -> row.getConsultation().getId())
                .distinct()
                .count();
    }
    ...
    public BigDecimal getTotalPrescriptionValue() {
        ...
        for (PrescriptionHistoryRow row : prescriptionRows) {
    ...
    public BigDecimal getAverageLineValue() {
        ...
        return getTotalPrescriptionValue().divide(BigDecimal.valueOf(prescriptionRows.size()), 2, RoundingMode.HALF_UP);
    }
    ...
    public String getMedicineMixJson() {
        Map<String, Long> mix = prescriptionRows.stream()
                .collect(Collectors.groupingBy(row -> {
                    ConsultationMedicine medicine = row.getConsultationMedicine();
                    String name = medicine.getMedicine() == null ? medicine.getDescriptionText() : medicine.getMedicine().getMedicineName();
                    return safeText(name, "Unknown Medicine");
                }, Collectors.counting()));
        if (mix.isEmpty()) {
            return "[]";
        }
    ...
    public String getDailyPrescriptionJson() {
    ...
    public String getPrescriptionValueJson() {
    ...
    public List<PrescriptionHistoryRow> getPrescriptionRows() {
    ...
    public static class PrescriptionHistoryRow implements Serializable {
